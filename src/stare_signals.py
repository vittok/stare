...
def market_sentiment_note(group: dict[str, Any]) -> str:
    ...
    return f"{direction.lower()} {group_kind} sentiment with strength {strength:.0f}"


def recommendation_for_stock(group: dict[str, Any], stock: dict[str, Any]) -> dict[str, Any]:
    ...
    score = raw_score * 1.5
    reasons = [market_sentiment_note(group)]
...
def decision_snapshot_for_stock(stock: dict[str, Any]) -> dict[str, Any]:
    fundamentals = stock.get("fundamentals") or {}
...
def summary_for_stock(group: dict[str, Any], stock: dict[str, Any]) -> str:
    fundamentals = stock.get("fundamentals") or {}
    recommendation = stock.get("recommendation") or {}
    ticker = stock.get("ticker") or "Ticker"
    name = _text(fundamentals.get("shortName")) or _text(fundamentals.get("industry")) or ticker
    group_name = group.get("sector") or group.get("region") or group.get("market") or "selected"
    group_kind = "regional" if group.get("region") else "sector"
    market_note = ""
...
def enrich_stock_signals(group: dict[str, Any], stock: dict[str, Any]) -> dict[str, Any]:
    stock["recommendation"] = recommendation_for_stock(group, stock)
    stock["decision_snapshot"] = decision_snapshot_for_stock(stock)
    stock["daily_summary"] = summary_for_stock(group, stock)
    return stock
